  ...
  generate() {
    setTimeout(async () => {
      try {
        ...
        let fileName = `${TENENT}_RYCS_${date.replace(/:/g, '_')}.txt`
...
const PersonaleLineFormatter = (personale: IPersonaleDoc) => {
  ...
  let { id, name, nationalId, jobTitle, department, isExternal, sex, education, maritalStatus, phone, address, certificates } = personale
